...
| **The second brain** | `delegate_local` hands a whole GOAL to a genuine peer agent that runs **entirely in this app**, not the worker: `electron/native/providers/local-react-agent.cjs` orchestrates its own ReAct loop on the laptop's Ollama model, with the SAME tool catalog the cloud router has (minus a few tools that would only ever recurse back into itself — `LOCAL_LOOP_DENY` in the worker's `router.ts`). `run_shell`/`run_code` execute natively, right here, through `sandbox-agent.cjs`'s existing boxed exec; every other tool is a plain HTTPS call to the worker's `POST /api/elle-tool`, authenticated with the same `ELLE_SANDBOX_KEY` the session bus uses. One job (`kind:'react_goal'`) carries the whole goal down; one result comes back when it's done. |
