...
| **atlas** | Her memory graph, in three dimensions (`react-force-graph-3d`). Node positions are computed entirely outside Elle, by a separate on-device repo that folds recall events into edges and pushes a versioned snapshot to the worker; this panel only ever `GET`s `/api/atlas/latest` (and `/history` / `/at` to scrub past snapshots) — the same read-only boundary the `atlas` router tool has. Gold edges are on a cycle (recurrence); oxblood edges are bridges. A snapshot may carry a `resonance` block — wormhole candidates scored by a learned embedder on-device (inference only); a deterministic client-side gate re-verifies each one and survivors render as dashed violet arcs behind a default-off toggle (see `docs/RESONANCE_CONTRACT.md`: the learned model proposes, the geometry disposes). |
